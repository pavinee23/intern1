...
export default function VietnamKoreaInvoicesPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        setError('');
        const res = await fetch('/api/korea-invoices?branch=vietnam&status=all', { cache: 'no-store' });
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 px-4 py-10">
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 flex items-center justify-between gap-3">
          <button
            type="button"
            onClick={() => router.push('/KR-Vietnam/Admin-Login/invoices/create')}
            className="inline-flex items-center rounded-lg border border-green-200 bg-white px-4 py-2 text-sm font-medium text-green-700 transition hover:border-green-300 hover:bg-green-50"
          >
            {isKo ? '← 돌아가기' : '← Back'}
          </button>
          ...
          <div className="inline-flex items-center rounded-xl border border-gray-200 bg-white p-1 shadow-sm">
            <button
              ...
              className={`rounded-lg px-3 py-2 text-sm font-semibold transition ${locale === 'ko' ? 'bg-green-500 text-white' : 'text-gray-600 hover:bg-gray-50'}`}
            >
              한국어
            </button>
            <button
              ...
              className={`rounded-lg px-3 py-2 text-sm font-semibold transition ${locale === 'en' ? 'bg-green-500 text-white' : 'text-gray-600 hover:bg-gray-50'}`}
            >
              English
            </button>
          </div>
            ...
            <div className="mb-8 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <p className="mt-2 text-base text-gray-600">
                  {isKo ? '본사에서 발행한 베트남 지점 인보이스' : 'Invoices from Korea HQ to Vietnam Branch'}
                </p>
              </div>
              <div className="rounded-2xl border border-gray-200 bg-gray-50 px-5 py-4 text-sm text-gray-600">
                ...
                <div className="mt-1 text-2xl font-bold text-green-600">{rows.length}</div>
              </div>
            </div>
              ...
              <div className="overflow-hidden rounded-2xl border border-gray-200">
                <div className="overflow-x-auto">
                  <table className="min-w-full bg-white">
                    ...
                    <tbody>
                      {rows.map((row) => (
                        <tr key={row.id} className="border-t border-gray-100 hover:bg-green-50/40">
