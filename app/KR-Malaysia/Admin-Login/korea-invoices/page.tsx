...
export default function MalaysiaKoreaInvoicesPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        setError('');
        const res = await fetch('/api/korea-invoices?branch=malaysia&status=all', { cache: 'no-store' });
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-violet-50 px-4 py-10">
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 flex items-center justify-between gap-3">
          <button
            type="button"
            onClick={() => router.push('/KR-Malaysia/Admin-Login/invoices/create')}
            className="inline-flex items-center rounded-lg border border-purple-200 bg-white px-4 py-2 text-sm font-medium text-purple-700 transition hover:border-purple-300 hover:bg-purple-50"
          >
            {isKo ? '← 돌아가기' : '← Back'}
          </button>
          ...
          <div className="inline-flex items-center rounded-xl border border-gray-200 bg-white p-1 shadow-sm">
            <button
              ...
              className={`rounded-lg px-3 py-2 text-sm font-semibold transition ${locale === 'ko' ? 'bg-purple-500 text-white' : 'text-gray-600 hover:bg-gray-50'}`}
            >
              한국어
            </button>
            <button
              ...
              className={`rounded-lg px-3 py-2 text-sm font-semibold transition ${locale === 'en' ? 'bg-purple-500 text-white' : 'text-gray-600 hover:bg-gray-50'}`}
            >
              English
            </button>
          </div>
        ...
        <section className="overflow-hidden rounded-3xl border border-purple-100 bg-white shadow-xl">
            ...
            <div className="mb-8 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <p className="mt-2 text-base text-gray-600">
                  {isKo ? '본사에서 발행한 말레이시아 지점 인보이스' : 'Invoices from Korea HQ to Malaysia Branch'}
                </p>
              </div>
              <div className="rounded-2xl border border-gray-200 bg-gray-50 px-5 py-4 text-sm text-gray-600">
                ...
                <div className="mt-1 text-2xl font-bold text-purple-600">{rows.length}</div>
              </div>
            </div>
                        ...
                        <tr key={row.id} className="border-t border-gray-100 hover:bg-purple-50/40">
                          ...
                          <td className="px-4 py-3 text-center">
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 rounded-lg bg-purple-500 px-3 py-1.5 text-xs font-medium text-white transition hover:bg-purple-600"
                            >
