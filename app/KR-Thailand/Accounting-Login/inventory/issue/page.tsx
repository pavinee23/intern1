...
export default function InventoryIssuePage(){
  ...
  const load=async()=>{const r=await fetch('/api/accounting/inventory'+"?move_type=out"+(search?'&q='+encodeURIComponent(search):''));const d=await r.json();if(d.ok)setData(d.data||[])}
  useEffect(()=>{load()},[])
