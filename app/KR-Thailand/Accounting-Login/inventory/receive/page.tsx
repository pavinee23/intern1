...
export default function InventoryReceivePage(){
  ...
  const load=async()=>{const r=await fetch('/api/accounting/inventory'+"?move_type=in"+(search?'&q='+encodeURIComponent(search):''));const d=await r.json();if(d.ok)setData(d.data||[])}
  useEffect(()=>{load()},[])
  ...
  return(<AccWindow title={L('Receive Goods','รับสินค้า')}><div style={{padding:12}}>
