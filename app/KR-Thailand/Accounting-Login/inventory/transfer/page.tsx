...
export default function InventoryTransferPage(){
  const{L}=useLang()
  const[data,setData]=useState<any[]>([]);const[form,setForm]=useState<any>({doc_no:'',move_date:new Date().toISOString().slice(0,10),product_name:'',qty:0,ref_doc:'',note:''});const[showForm,setShowForm]=useState(false)
  const[loading,setLoading]=useState(false);const[search,setSearch]=useState('');const[msg,setMsg]=useState('')
  const load=async()=>{const r=await fetch('/api/accounting/inventory'+"?move_type=transfer"+(search?'&q='+encodeURIComponent(search):''));const d=await r.json();if(d.ok)setData(d.data||[])}
  useEffect(()=>{load()},[])
  const save=async()=>{setLoading(true);setMsg('');const m=form.id?'PUT':'POST';const r=await fetch('/api/accounting/inventory',{method:m,headers:{'Content-Type':'application/json'},body:JSON.stringify(form)});const d=await r.json();if(d.ok){setShowForm(false);setForm({doc_no:'',move_date:new Date().toISOString().slice(0,10),product_name:'',qty:0,ref_doc:'',note:''});load();setMsg(L('Saved','บันทึกสำเร็จ'))}else setMsg('Error: '+d.error);setLoading(false)}
  const del=async(id:number)=>{if(!confirm(L('Delete?','ลบ?')))return;await fetch('/api/accounting/inventory?id='+id,{method:'DELETE'});load()}
  return(<AccWindow title={L('Transfer Goods','โอนสินค้า')}><div style={{padding:12}}>
    <div style={{display:'flex',gap:6,marginBottom:8,alignItems:'center',flexWrap:'wrap'}}>
      <button style={btn('#f3f4f6')} onClick={()=>{setForm({doc_no:'',move_date:new Date().toISOString().slice(0,10),product_name:'',qty:0,ref_doc:'',note:''});setShowForm(true)}}>+ {L('New','เพิ่มใหม่')}</button>
