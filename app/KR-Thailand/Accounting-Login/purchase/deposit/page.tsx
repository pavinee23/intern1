...
export default function PurchaseDepositPage() {
  const { L } = useLang()
  ...
  const load = async () => { const r = await fetch('/api/accounting/purchase-orders?doc_type=deposit' + (search ? '&q=' + encodeURIComponent(search) : '')); const d = await r.json(); if (d.ok) setData(d.data) }
  useEffect(() => { load() }, [])

  const save = async () => {
    setLoading(true); setMsg('')
    const method = form.id ? 'PUT' : 'POST'
    const res = await fetch('/api/accounting/purchase-orders', { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, doc_type: 'deposit' }) })
  ...
  return (
    <AccWindow title={L('Pay Deposit','จ่ายเงินมัดจำ')}>
      <div style={{ padding: 12 }}>
        ...
        <div style={{ overflowX: 'auto', border: '1px solid #d1d5db', borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 6px rgba(0,0,0,0.08)' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr>{[L('Doc No','เลขที่'), L('Date','วันที่'), L('Supplier','ผู้จำหน่าย'), L('Amount','จำนวนเงิน'), L('Status','สถานะ'), ''].map((h, i) => <th key={i} style={th}>{h}</th>)}</tr></thead>
            <tbody>
              ...
              {data.map((r: any, i) => (
                <tr key={r.id} style={{ background: i % 2 ? '#f5f5f5' : '#fff' }}>
                  ...
                  <td style={{ ...td, textAlign: 'right' }}>{fmt(r.total || 0)}</td><td style={{ ...td, textAlign: 'center' }}>{r.status}</td>
        ...
        {showForm && <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', border: '1px solid #d1d5db', borderRadius: 12, boxShadow: '0 20px 60px rgba(0,0,0,0.3)', minWidth: 460, maxWidth: 560, width: '95%' }}>
            <div style={{ background: 'linear-gradient(135deg, #374151 0%, #4b5563 100%)', padding: '3px 8px', color: '#fff', fontSize: 13, fontWeight: 700, display: 'flex', justifyContent: 'space-between' }}>
              <span>{form.id ? L('Edit','แก้ไข') : L('New Deposit','เพิ่มมัดจำ')}</span><span style={{ cursor: 'pointer' }} onClick={() => setShowForm(false)}>✕</span>
            </div>
            <div style={{ padding: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 12px' }}>
              ...
              <div><div style={{ fontSize: 12, marginBottom: 2 }}>{L('Amount','จำนวนเงิน')}</div><input style={inp} type="number" value={form.total || ''} onChange={e => setForm(f => ({ ...f, total: +e.target.value || 0, subtotal: +e.target.value || 0 }))} /></div>
