...
type PayableRow = { name?: string; balance?: number }
...
export default function RecalcPayablePage(){
  const{L}=useLang()
  const[data,setData]=useState<PayableRow[]>([])
  useEffect(()=>{fetch('/api/accounting/reports?type=ar-ap').then(r=>r.json()).then(d=>{if(d.ok)setData((d.data?.payables||[]) as PayableRow[])})},[])
  return(<AccWindow title={L('Recalculate AP','คำนวณยอดเจ้าหนี้ใหม่')}><div style={{padding:12}}>
    <div style={{overflowX:'auto',border:'1px solid #d1d5db',borderRadius:12,overflow:'hidden',boxShadow:'0 2px 6px rgba(0,0,0,0.08)'}}><table style={{width:'100%',borderCollapse:'collapse'}}>
      <thead><tr><th style={th}>{L('Supplier','ผู้จำหน่าย')}</th><th style={th}>{L('Balance','ยอดค้าง')}</th></tr></thead>
      <tbody>{data.length===0&&<tr><td colSpan={2} style={{...td,textAlign:'center',color:'#888',padding:20}}>{L('No data','ไม่มีข้อมูล')}</td></tr>}
        {data.map((r,i)=>(<tr key={i} style={{background:i%2?'#f5f5f5':'#fff'}}>
