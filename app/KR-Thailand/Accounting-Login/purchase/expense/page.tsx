      ...
      <div style={{ padding: 12 }}>
        ...
        <div style={{ overflowX: 'auto', border: '1px solid #d1d5db', borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 6px rgba(0,0,0,0.08)' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr>
              {[L('Doc No', 'เลขที่'), L('Date', 'วันที่'), L('Description', 'รายละเอียด'), L('Amount', 'จำนวนเงิน'), L('Method', 'วิธี'), L('Status', 'สถานะ'), ''].map((h, i) => <th key={i} style={th}>{h}</th>)}
            </tr></thead>
            <tbody>
              ...
              {data.map((r, i) => (
                <tr key={r.id} style={{ background: i % 2 ? '#f5f5f5' : '#fff' }}>
                  ...
                  <td style={{ ...td, textAlign: 'right' }}>{fmt(r.amount || 0)}</td>
                  <td style={{ ...td, textAlign: 'center' }}>{r.method}</td>
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: '#fff', border: '1px solid #d1d5db', borderRadius: 12, boxShadow: '0 20px 60px rgba(0,0,0,0.3)', minWidth: 420, maxWidth: 520, width: '95%' }}>
              <div style={{ background: 'linear-gradient(135deg, #374151 0%, #4b5563 100%)', padding: '3px 8px', color: '#fff', fontSize: 13, fontWeight: 700, display: 'flex', justifyContent: 'space-between' }}>
                <span>{form.id ? L('Edit', 'แก้ไข') : L('New Expense', 'เพิ่มค่าใช้จ่าย')}</span>
              ...
              <div style={{ padding: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 12px' }}>
                ...
                <div><div style={{ fontSize: 12, marginBottom: 2 }}>{L('Amount', 'จำนวนเงิน')}</div><input style={inp} type="number" value={form.amount || ''} onChange={e => setForm(f => ({ ...f, amount: +e.target.value || 0 }))} /></div>
                <div><div style={{ fontSize: 12, marginBottom: 2 }}>{L('Method', 'วิธีชำระ')}</div>
                  <select style={inp} value={form.method || 'cash'} onChange={e => setForm(f => ({ ...f, method: e.target.value }))}>
                    <option value="cash">{L('Cash', 'เงินสด')}</option>
                    <option value="transfer">{L('Transfer', 'โอน')}</option>
                ...
                <div style={{ gridColumn: 'span 2' }}><div style={{ fontSize: 12, marginBottom: 2 }}>{L('Description', 'รายละเอียด')}</div>
                  <textarea style={{ ...inp, height: 60, resize: 'vertical' }} value={form.description || ''} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
                </div>
              </div>
