...
export default function ExpenseCatPage(){
  ...
  const load=async()=>{const r=await fetch('/api/accounting/expense-categories'+(search?'?q='+encodeURIComponent(search):''));const d=await r.json();if(d.ok)setData(d.data||[])}
  useEffect(()=>{load()},[])
  ...
  const del=async(id:number)=>{if(!confirm(L('Delete?','ลบ?')))return;await fetch('/api/accounting/expense-categories?id='+id,{method:'DELETE'});load()}
