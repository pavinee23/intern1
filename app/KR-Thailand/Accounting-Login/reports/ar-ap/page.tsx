...
export default function ARAPReportPage(){
  const{L}=useLang()
  const[ar,setAr]=useState<any[]>([]);const[ap,setAp]=useState<any[]>([])
  useEffect(()=>{fetch('/api/accounting/reports?type=ar-ap').then(r=>r.json()).then(d=>{if(d.ok){setAr(d.data?.receivables||[]);setAp(d.data?.payables||[])}})},[])
  const Section=({title,titleTh,data:rows}:{title:string,titleTh:string,data:any[]})=>(<div style={{marginBottom:16}}>
    ...
    <div style={{border:'1px solid #d1d5db',borderRadius:12,overflow:'hidden',boxShadow:'0 2px 6px rgba(0,0,0,0.08)'}}><table style={{width:'100%',borderCollapse:'collapse'}}>
      ...
      <tbody>{rows.length===0&&<tr><td colSpan={2} style={{...td,textAlign:'center',color:'#888',padding:16}}>{L('No data','ไม่มีข้อมูล')}</td></tr>}
        {rows.map((r:any,i:number)=>(<tr key={i} style={{background:i%2?'#f5f5f5':'#fff'}}>
          <td style={td}>{r.name}</td><td style={{...td,textAlign:'right'}}>{fmt(r.balance||0)}</td>
        </tr>))}<tr style={{background:'#e8e8e8'}}><td style={{...td,fontWeight:700}}>{L('Total','รวม')}</td><td style={{...td,textAlign:'right',fontWeight:700}}>{fmt(rows.reduce((s:number,r:any)=>s+(r.balance||0),0))}</td></tr>
      </tbody></table></div></div>)
