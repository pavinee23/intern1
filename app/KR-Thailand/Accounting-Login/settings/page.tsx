...
export default function SettingsPage(){
  const{L}=useLang()
  const[data,setData]=useState<any[]>([]);const[editRow,setEditRow]=useState<any>(null);const[val,setVal]=useState('')
  const load=async()=>{const r=await fetch('/api/accounting/settings');const d=await r.json();if(d.ok)setData(d.data||[])}
  useEffect(()=>{load()},[])
  ...
  return(<AccWindow title={L('System Settings','ตั้งค่าระบบ')}><div style={{padding:12}}>
    <div style={{overflowX:'auto',border:'1px solid #d1d5db',borderRadius:12,overflow:'hidden',boxShadow:'0 2px 6px rgba(0,0,0,0.08)'}}><table style={{width:'100%',borderCollapse:'collapse'}}>
      ...
      <tbody>{data.map((r:any,i:number)=>(<tr key={r.id} style={{background:i%2?'#f5f5f5':'#fff'}}>
