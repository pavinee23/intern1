...
interface PaymentVoucher {
  ...
  method: string
  status: string
  korea_invoice_id: string
  description: string
}

const createEmptyForm = (): PaymentVoucher => ({
  doc_no:'',
...
export default function MakePaymentPage(){
  ...
  const[data,setData]=useState<PaymentVoucher[]>([]);const[form,setForm]=useState<PaymentVoucher>(createEmptyForm);const[showForm,setShowForm]=useState(false)
  const[loading,setLoading]=useState(false);const[search,setSearch]=useState('');const[msg,setMsg]=useState('')
  const load=useCallback(async()=>{const r=await fetch('/api/accounting/payment-vouchers'+"?voucher_type=pay&party_type=supplier"+(search?'&q='+encodeURIComponent(search):''));const d=await r.json();if(d.ok)setData((d.data||[]) as PaymentVoucher[])},[search])
  const generateDocNo=useCallback(async()=>{
    const r=await fetch('/api/accounting/payment-vouchers');
    const d=await r.json();
    if(d.ok){
      const vouchers=(d.data||[]) as PaymentVoucher[]
      ...
      const count=vouchers.filter((v)=>v.doc_no?.startsWith(prefix)).length||0;
      const newNo=`${prefix}-${String(count+1).padStart(4,'0')}`;
      setForm((f)=>({...f,doc_no:newNo}))
    }
  },[])
  useEffect(()=>{
    void load()
  ...
  },[generateDocNo, load, searchParams])
  ...
  return(<AccWindow title={L('Make Payment','จ่ายชำระหนี้')}><div style={{padding:12}}>
    <div style={{display:'flex',gap:6,marginBottom:8,alignItems:'center',flexWrap:'wrap'}}>
      <button style={btn('#f3f4f6')} onClick={()=>{setForm(createEmptyForm());setShowForm(true);void generateDocNo()}}>+ {L('New','เพิ่มใหม่')}</button>
    ...
    {showForm&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:2000,display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{background:'#fff',border:'1px solid #d1d5db',borderRadius:12,boxShadow:'0 20px 60px rgba(0,0,0,0.3)',minWidth:420,maxWidth:650,width:'95%'}}>
        ...
        <div style={{padding:16,display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px 12px'}}>
          <div><div style={{fontSize:12,marginBottom:2}}>{L('Doc No','เลขที่')}</div><div style={{display:'flex',gap:4}}><input style={{...inp,flex:1}} value={form.doc_no||''} onChange={(e)=>setForm((f2)=>({...f2,doc_no:e.target.value}))} /><button type="button" onClick={generateDocNo} style={{...btn('#f3f4f6'),padding:'7px 12px',flexShrink:0}} title={L('Refresh','รีเฟรช')}>🔄</button></div></div>
          ...
          <div style={{gridColumn:'1 / -1'}}><div style={{fontSize:12,marginBottom:2}}>{form.korea_invoice_id?L('Branch','สาขา'):L('Supplier','ผู้จำหน่าย')}</div><input style={inp} value={form.party_name||''} onChange={(e)=>setForm((f2)=>({...f2,party_name:e.target.value}))} /></div>
          ...
          <div><div style={{fontSize:12,marginBottom:2}}>{L('Method','วิธีชำระ')}</div><select style={inp} value={form.method||'transfer'} onChange={(e)=>setForm((f2)=>({...f2,method:e.target.value}))}><option value="">-</option><option value="cash">{L('Cash','เงินสด')}</option><option value="transfer">{L('Transfer','โอนเงิน')}</option><option value="cheque">{L('Cheque','เช็ค')}</option></select></div>
          <div><div style={{fontSize:12,marginBottom:2}}>{L('Status','สถานะ')}</div><select style={inp} value={form.status||'draft'} onChange={(e)=>setForm((f2)=>({...f2,status:e.target.value}))}><option value="draft">{L('Draft','ร่าง')}</option><option value="posted">{L('Posted','บันทึกแล้ว')}</option></select></div>
          <div style={{gridColumn:'1 / -1'}}><div style={{fontSize:12,marginBottom:2}}>{L('Description','รายละเอียด')}</div><textarea style={{...inp,minHeight:60,resize:'vertical'}} value={form.description||''} onChange={(e)=>setForm((f2)=>({...f2,description:e.target.value}))} /></div>
        </div>
