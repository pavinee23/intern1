...
export default function BankReconcilePage(){
  const{L}=useLang()
  const[data,setData]=useState<any[]>([]);const[form,setForm]=useState<any>({recon_date:new Date().toISOString().slice(0,10),statement_balance:0,book_balance:0,difference:0,status:''});const[showForm,setShowForm]=useState(false)
  const[loading,setLoading]=useState(false);const[search,setSearch]=useState('');const[msg,setMsg]=useState('')
  const load=async()=>{const r=await fetch('/api/accounting/bank-reconciliation'+(search?'?q='+encodeURIComponent(search):''));const d=await r.json();if(d.ok)setData(d.data||[])}
  useEffect(()=>{load()},[])
  const save=async()=>{setLoading(true);setMsg('');const m=form.id?'PUT':'POST';const r=await fetch('/api/accounting/bank-reconciliation',{method:m,headers:{'Content-Type':'application/json'},body:JSON.stringify(form)});const d=await r.json();if(d.ok){setShowForm(false);setForm({recon_date:new Date().toISOString().slice(0,10),statement_balance:0,book_balance:0,difference:0,status:''});load();setMsg(L('Saved','บันทึกสำเร็จ'))}else setMsg('Error: '+d.error);setLoading(false)}
  const del=async(id:number)=>{if(!confirm(L('Delete?','ลบ?')))return;await fetch('/api/accounting/bank-reconciliation?id='+id,{method:'DELETE'});load()}
  return(<AccWindow title={L('Bank Reconciliation','กระทบยอดเงินฝาก')}><div style={{padding:12}}>
    <div style={{display:'flex',gap:6,marginBottom:8,alignItems:'center',flexWrap:'wrap'}}>
      <button style={btn('#f3f4f6')} onClick={()=>{setForm({recon_date:new Date().toISOString().slice(0,10),statement_balance:0,book_balance:0,difference:0,status:''});setShowForm(true)}}>+ {L('New','เพิ่มใหม่')}</button>
    ...
    <div style={{overflowX:'auto',border:'1px solid #d1d5db',borderRadius:12,overflow:'hidden',boxShadow:'0 2px 6px rgba(0,0,0,0.08)'}}><table style={{width:'100%',borderCollapse:'collapse'}}>
      <thead><tr>{[L('Date','วันที่'),L('Statement','ยอดตามเอกสาร'),L('Book','ยอดตามบัญชี'),L('Difference','ผลต่าง'),L('Status','สถานะ'),''].map((h,i)=><th key={i} style={th}>{h}</th>)}</tr></thead>
      <tbody>{data.length===0&&<tr><td colSpan={6} style={{...td,textAlign:'center',color:'#888',padding:20}}>{L('No data','ไม่มีข้อมูล')}</td></tr>}
        {data.map((r:any,i)=>(<tr key={r.id} style={{background:i%2?'#f5f5f5':'#fff'}}>
          <td style={td}>{r.recon_date?.slice(0,10)}</td><td style={{...td,textAlign:'right'}}>{fmt(r.statement_balance||0)}</td><td style={{...td,textAlign:'right'}}>{fmt(r.book_balance||0)}</td><td style={{...td,textAlign:'right'}}>{fmt(r.difference||0)}</td><td style={{...td,textAlign:'center'}}>{r.status}</td>
    ...
    {showForm&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:2000,display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{background:'#fff',border:'1px solid #d1d5db',borderRadius:12,boxShadow:'0 20px 60px rgba(0,0,0,0.3)',minWidth:420,maxWidth:520,width:'95%'}}>
        ...
        <div style={{padding:16,display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px 12px'}}>
          <div><div style={{fontSize:12,marginBottom:2}}>{L('Date','วันที่')}</div><input style={inp} type="date" value={form.recon_date||''} onChange={e=>setForm(f2=>({...f2,recon_date:e.target.value}))} /></div>
              <div><div style={{fontSize:12,marginBottom:2}}>{L('Statement','ยอดตามเอกสาร')}</div><input style={inp} type="number" value={form.statement_balance||''} onChange={e=>setForm(f2=>({...f2,statement_balance:+e.target.value||0}))} /></div>
              <div><div style={{fontSize:12,marginBottom:2}}>{L('Book','ยอดตามบัญชี')}</div><input style={inp} type="number" value={form.book_balance||''} onChange={e=>setForm(f2=>({...f2,book_balance:+e.target.value||0}))} /></div>
              <div><div style={{fontSize:12,marginBottom:2}}>{L('Difference','ผลต่าง')}</div><input style={inp} type="number" value={form.difference||''} onChange={e=>setForm(f2=>({...f2,difference:+e.target.value||0}))} /></div>
