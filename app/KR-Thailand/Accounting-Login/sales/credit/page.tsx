...
export default function SalesCreditPage(){
  ...
  const load=async()=>{const r=await fetch('/api/accounting/sales-invoices'+"?doc_type=credit"+(search?'&q='+encodeURIComponent(search):''));const d=await r.json();if(d.ok)setData(d.data||[])}
  useEffect(()=>{load()},[])
  ...
  return(<AccWindow title={L('Credit Sales','ขายเงินเชื่อ')}><div style={{padding:12}}>
    ...
    <div style={{overflowX:'auto',border:'1px solid #d1d5db',borderRadius:12,overflow:'hidden',boxShadow:'0 2px 6px rgba(0,0,0,0.08)'}}><table style={{width:'100%',borderCollapse:'collapse'}}>
      <thead><tr>{[L('Doc No','เลขที่'),L('Date','วันที่'),L('Customer','ลูกค้า'),L('Total','รวม'),L('Status','สถานะ'),''].map((h,i)=><th key={i} style={th}>{h}</th>)}</tr></thead>
