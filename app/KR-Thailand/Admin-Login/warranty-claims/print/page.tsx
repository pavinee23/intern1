...
function WarrantyClaimPrintContent() {
  ...
  useEffect(() => {
    const idOrNo = wcID || wcNo
    if (!idOrNo) return
    ;(async () => {
      try {
        let res = await fetch(`/api/warranty-claims?wcID=${encodeURIComponent(wcID)}`)
        let j = await res.json().catch(() => null)
        if (!(res.ok && j && j.success && j.warrantyClaim)) {
          res = await fetch(`/api/warranty-claims?wcNo=${encodeURIComponent(wcNo || wcID)}`)
          j = await res.json().catch(() => null)
        }
        if (j && j.success) {
          const q = j.warrantyClaim || j.warrantyClaims?.[0] || null
        ...
        console.error('Failed to load warranty claim for print', err)
      }
    })()
  }, [wcID, wcNo])
  ...
  useEffect(() => {
    ...
    const key = `print_count:warranty-claim:${wcID || wcNo || 'unknown'}`
  ...
  if (!wcID && !wcNo) return <div style={{ padding: 20 }}>Missing wcID or wcNo</div>
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .summary-row.total { font-weight: 700; font-size: 12pt; color: #7c3aed; border-top: 2px solid #7c3aed; border-bottom: none; padding-top: 10px; margin-top: 4px; }
      ...
      <div className="a4-page">
        <div className="header-row">
          ...
          <div className="doc-title">
            <h1>{L('WARRANTY CLAIM', 'ใบรับประกัน')}</h1>
            <h2>{L('Warranty Document', 'เอกสารรับประกัน')}</h2>
          </div>
        </div>

        <div className="info-section">
          <div className="info-box">
            <div className="info-box-title">{L('Warranty Claim Information', 'ข้อมูลใบรับประกัน')}</div>
            <div className="info-row">
              <span className="info-label">{L('WC No:', 'เลขที่:')}</span>
              <span className="info-value" style={{ fontWeight: 700 }}>{doc.wcNo || doc.wc_no || '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Date:', 'วันที่:')}</span>
              <span className="info-value">{doc.wcDate || doc.date ? new Date(doc.wcDate || doc.date).toLocaleDateString(selectedLang === 'th' ? 'th-TH' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Invoice No:', 'ใบแจ้งหนี้:')}</span>
              <span className="info-value">{doc.invoice_no || doc.reference || '-'}</span>
            </div>
            <div className="info-row">
        ...
        <div className="signature-section">
          <div className="signature-box">
            ...
            <div className="signature-sublabel">{L('Service', 'ฝ่ายบริการ')}</div>
          </div>
          <div className="signature-box">
            <div className="signature-line"></div>
...
export default function WarrantyClaimPrintPage() {
  return (
    <Suspense fallback={<div style={{ padding: 20, textAlign: 'center' }}>Loading...</div>}>
      <WarrantyClaimPrintContent />
    </Suspense>
  )
}
