...
function ExpenseBillPrintContent() {
  const searchParams = useSearchParams()
  const ebID = searchParams?.get('ebID') || ''
  ...
  useEffect(() => {
    const idOrNo = ebID || ebNo
    if (!idOrNo) return
    ;(async () => {
      try {
        let res = await fetch(`/api/expense-bills?ebID=${encodeURIComponent(ebID)}`)
        let j = await res.json().catch(() => null)
        if (!(res.ok && j && j.success && j.expenseBill)) {
          res = await fetch(`/api/expense-bills?ebNo=${encodeURIComponent(ebNo || ebID)}`)
          j = await res.json().catch(() => null)
        }
        if (j && j.success) {
          const q = j.expenseBill || j.expenseBills?.[0] || null
        ...
        console.error('Failed to load expense bill for print', err)
      }
    })()
  }, [ebID, ebNo])
  ...
  useEffect(() => {
    ...
    const key = `print_count:expense-bill:${ebID || ebNo || 'unknown'}`
  ...
  if (!ebID && !ebNo) return <div style={{ padding: 20 }}>Missing ebID or ebNo</div>
  ...
  const expenseCategory = doc.expense_category || doc.category || '-'
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .doc-title h1 { font-size: 22pt; font-weight: 700; color: #dc2626; margin: 0 0 4px 0; }
        ...
        .items-table th { background: #dc2626; color: white; padding: 8px 10px; text-align: left; font-weight: 600; }
        ...
        .summary-row.total { font-weight: 700; font-size: 12pt; color: #dc2626; border-top: 2px solid #dc2626; border-bottom: none; padding-top: 10px; margin-top: 4px; }
      ...
      <div className="a4-page">
        <div className="header-row">
          ...
          <div className="doc-title">
            <h1>{L('EXPENSE BILL', 'บิลค่าใช้จ่าย')}</h1>
            <h2>{L('Expense Document', 'เอกสารค่าใช้จ่าย')}</h2>
          </div>
        </div>

        <div className="info-section">
          <div className="info-box">
            <div className="info-box-title">{L('Expense Bill Information', 'ข้อมูลบิลค่าใช้จ่าย')}</div>
            <div className="info-row">
              <span className="info-label">{L('Bill No:', 'เลขที่:')}</span>
              <span className="info-value" style={{ fontWeight: 700 }}>{doc.ebNo || doc.eb_no || '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Date:', 'วันที่:')}</span>
              <span className="info-value">{doc.ebDate || doc.date ? new Date(doc.ebDate || doc.date).toLocaleDateString(selectedLang === 'th' ? 'th-TH' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Category:', 'หมวดหมู่:')}</span>
              <span className="info-value">{expenseCategory}</span>
            </div>
            <div className="info-row">
          ...
          <div className="info-box">
            ...
            <div className="info-row">
              <span className="info-label">{L('Payee:', 'ผู้รับเงิน:')}</span>
            ...
            <div className="info-row">
              <span className="info-label">{L('Description:', 'รายละเอียด:')}</span>
...
export default function ExpenseBillPrintPage() {
  return (
    <Suspense fallback={<div style={{ padding: 20, textAlign: 'center' }}>Loading...</div>}>
      <ExpenseBillPrintContent />
    </Suspense>
  )
}
