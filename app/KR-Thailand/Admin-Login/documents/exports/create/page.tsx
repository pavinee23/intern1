...
export default function CreateExportPage() {
  ...
  function validate() {
    ...
    if (!destinationCountry) errs.push(L('Destination country is required', 'กรุณาระบุประเทศปลายทาง'))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok && j.success) {
        setMessageBar({ type: 'success', text: L('Export created successfully', 'สร้างใบส่งออกสำเร็จ') })
        setTimeout(() => router.push('/KR-Thailand/Admin-Login/documents/exports'), 900)
      } else {
  ...
  return (
    <AdminLayout title="Create Export" titleTh="สร้างใบส่งออก">
      {messageBar && (
        <div style={{
          padding: '12px 16px',
          marginBottom: '16px',
      ...
      <div className={styles.contentCard}>
        <div className={styles.cardHeader}>
          <h2 className={styles.cardTitle}>{L('Create Export Document', 'สร้างใบส่งออก')}</h2>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className={styles.formRow}>
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>{L('Export No.', 'เลขที่ส่งออก')}</label>
                <input type="text" value={expNo || L('Auto-generated', 'สร้างอัตโนมัติ')} disabled className={styles.formInput} style={{ background: '#f5f5f5' }} />
              </div>
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>{L('Date', 'วันที่')}</label>
                <input type="date" value={expDate} onChange={e => setExpDate(e.target.value)} className={styles.formInput} />
              </div>
            </div>

            <div className={styles.formRow}>
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>{L('Customer Name', 'ชื่อลูกค้า')} <span style={{ color: '#dc2626' }}>*</span></label>
                <input type="text" value={customerName} onChange={e => setCustomerName(e.target.value)} className={styles.formInput} placeholder={L('Customer name', 'ชื่อลูกค้า')} />
              </div>
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>{L('Destination Country', 'ประเทศปลายทาง')} <span style={{ color: '#dc2626' }}>*</span></label>
                <input type="text" value={destinationCountry} onChange={e => setDestinationCountry(e.target.value)} className={styles.formInput} />
              </div>
            </div>

            <div className={styles.formGroup}>
              <label className={styles.formLabel}>{L('Destination Address', 'ที่อยู่ปลายทาง')}</label>
              <textarea value={destinationAddress} onChange={e => setDestinationAddress(e.target.value)} className={styles.formInput} rows={2} />
            </div>

            <div className={styles.formRow}>
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>{L('Warehouse', 'คลังสินค้า')} <span style={{ color: '#dc2626' }}>*</span></label>
                <input type="text" value={warehouse} onChange={e => setWarehouse(e.target.value)} className={styles.formInput} placeholder={L('Source warehouse', 'คลังต้นทาง')} />
              </div>
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>{L('Shipper Name', 'ชื่อผู้ส่ง')}</label>
                <input type="text" value={shipperName} onChange={e => setShipperName(e.target.value)} className={styles.formInput} />
              </div>
            </div>
            ...
            <div className={styles.formRow}>
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>{L('Shipping Company', 'บริษัทขนส่ง')}</label>
                <input type="text" value={shippingCompany} onChange={e => setShippingCompany(e.target.value)} className={styles.formInput} />
              </div>
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>{L('Tracking No.', 'เลขติดตาม')}</label>
                <input type="text" value={trackingNo} onChange={e => setTrackingNo(e.target.value)} className={styles.formInput} />
              </div>
            </div>
            ...
            <h3 style={{ marginTop: 30, marginBottom: 15, fontSize: 16, fontWeight: 600 }}>{L('Export Items', 'รายการสินค้าส่งออก')}</h3>

            <div style={{ overflowX: 'auto' }}>
              <table className={styles.table}>
                <thead>
                  <tr>
                    <th style={{ width: '12%' }}>{L('Code', 'รหัส')}</th>
                    <th style={{ width: '18%' }}>{L('Product', 'สินค้า')}</th>
                    <th style={{ width: '18%' }}>{L('Description', 'รายละเอียด')}</th>
                    <th style={{ width: '10%' }}>{L('Qty', 'จำนวน')}</th>
                    <th style={{ width: '8%' }}>{L('Unit', 'หน่วย')}</th>
                    <th style={{ width: '12%' }}>{L('Price', 'ราคา')}</th>
                    <th style={{ width: '12%' }}>{L('Total', 'รวม')}</th>
                    <th style={{ width: '12%' }}>{L('HS Code', 'HS Code')}</th>
                    <th style={{ width: '4%' }}></th>
                  </tr>
                </thead>
                <tbody>
