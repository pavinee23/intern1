...
function StockCardPrintContent() {
  ...
  useEffect(() => {
    const idOrNo = scID || scNo
    if (!idOrNo) return
    ;(async () => {
      try {
        let res = await fetch(`/api/stock-cards?scID=${encodeURIComponent(scID)}`)
        let j = await res.json().catch(() => null)
        if (!(res.ok && j && j.success && j.stockCard)) {
          res = await fetch(`/api/stock-cards?scNo=${encodeURIComponent(scNo || scID)}`)
          j = await res.json().catch(() => null)
        }
        if (j && j.success) {
          const q = j.stockCard || j.stockCards?.[0] || null
        ...
        console.error('Failed to load stock card for print', err)
      }
    })()
  }, [scID, scNo])
  ...
  useEffect(() => {
    ...
    const key = `print_count:stock-card:${scID || scNo || 'unknown'}`
  ...
  if (!scID && !scNo) return <div style={{ padding: 20 }}>Missing scID or scNo</div>
  ...
  const productCode = doc.product_code || doc.sku || '-'
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .doc-title h1 { font-size: 22pt; font-weight: 700; color: #059669; margin: 0 0 4px 0; }
        ...
        .items-table th { background: #059669; color: white; padding: 8px 10px; text-align: left; font-weight: 600; }
        ...
        .summary-row.total { font-weight: 700; font-size: 12pt; color: #059669; border-top: 2px solid #059669; border-bottom: none; padding-top: 10px; margin-top: 4px; }
      ...
      <div className="a4-page">
        <div className="header-row">
          ...
          <div className="doc-title">
            <h1>{L('STOCK CARD', 'การ์ดสินค้า')}</h1>
            <h2>{L('Inventory Card', 'บัตรคุมสต๊อก')}</h2>
          </div>
        </div>

        <div className="info-section">
          <div className="info-box">
            <div className="info-box-title">{L('Stock Card Information', 'ข้อมูลการ์ดสินค้า')}</div>
            <div className="info-row">
              <span className="info-label">{L('Card No:', 'เลขที่:')}</span>
              <span className="info-value" style={{ fontWeight: 700 }}>{doc.scNo || doc.sc_no || '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Date:', 'วันที่:')}</span>
              <span className="info-value">{doc.scDate || doc.date ? new Date(doc.scDate || doc.date).toLocaleDateString(selectedLang === 'th' ? 'th-TH' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : '-'}</span>
            </div>
            <div className="info-row">
            ...
            <div className="info-row">
              ...
              <span className="info-value" style={{ color: doc.status === 'active' ? '#16a34a' : '#666' }}>{doc.status || 'active'}</span>
            </div>
          </div>
          <div className="info-box">
            ...
            <div className="info-row">
              <span className="info-label">{L('Code:', 'รหัส:')}</span>
              <span className="info-value">{productCode}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Unit:', 'หน่วย:')}</span>
              <span className="info-value">{unit}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Balance:', 'คงเหลือ:')}</span>
              <span className="info-value">{doc.balance || doc.stock_balance || '0'}</span>
            </div>
          </div>
        </div>
