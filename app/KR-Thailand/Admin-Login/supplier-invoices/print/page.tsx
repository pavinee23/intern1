...
function SupplierInvoicePrintContent() {
  ...
  useEffect(() => {
    const idOrNo = siID || siNo
    if (!idOrNo) return
    ;(async () => {
      try {
        let res = await fetch(`/api/supplier-invoices?siID=${encodeURIComponent(siID)}`)
        let j = await res.json().catch(() => null)
        if (!(res.ok && j && j.success && j.supplierInvoice)) {
          res = await fetch(`/api/supplier-invoices?siNo=${encodeURIComponent(siNo || siID)}`)
          j = await res.json().catch(() => null)
        }
        if (j && j.success) {
          const q = j.supplierInvoice || j.supplierInvoices?.[0] || null
        ...
        console.error('Failed to load supplier invoice for print', err)
      }
    })()
  }, [siID, siNo])
  ...
  useEffect(() => {
    ...
    const key = `print_count:supplier-invoice:${siID || siNo || 'unknown'}`
  ...
  if (!siID && !siNo) return <div style={{ padding: 20 }}>Missing siID or siNo</div>
  ...
  const supplierName = doc.supplier_name || doc.supplier || '-'
  const supplierAddress = doc.supplier_address || doc.address || '-'
  const supplierPhone = doc.supplier_phone || doc.phone || '-'
  const supplierEmail = doc.supplier_email || doc.email || '-'
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .doc-title h1 { font-size: 22pt; font-weight: 700; color: #ea580c; margin: 0 0 4px 0; }
        ...
        .items-table th { background: #ea580c; color: white; padding: 8px 10px; text-align: left; font-weight: 600; }
        ...
        .summary-row.total { font-weight: 700; font-size: 12pt; color: #ea580c; border-top: 2px solid #ea580c; border-bottom: none; padding-top: 10px; margin-top: 4px; }
      ...
      <div className="a4-page">
        <div className="header-row">
          ...
          <div className="doc-title">
            <h1>{L('SUPPLIER INVOICE', 'ใบแจ้งหนี้ผู้ขาย')}</h1>
            <h2>{L('Payable Invoice', 'ใบแจ้งหนี้')}</h2>
          </div>
        </div>
        ...
        <div className="info-section">
          <div className="info-box">
            ...
            <div className="info-row">
              ...
              <span className="info-value" style={{ fontWeight: 700 }}>{doc.siNo || doc.si_no || doc.invoice_no || '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Date:', 'วันที่:')}</span>
              <span className="info-value">{doc.siDate || doc.date ? new Date(doc.siDate || doc.date).toLocaleDateString(selectedLang === 'th' ? 'th-TH' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Due Date:', 'ครบกำหนด:')}</span>
              <span className="info-value">{doc.due_date || doc.dueDate ? new Date(doc.due_date || doc.dueDate).toLocaleDateString(selectedLang === 'th' ? 'th-TH' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Status:', 'สถานะ:')}</span>
              <span className="info-value" style={{ color: doc.status === 'paid' ? '#16a34a' : '#666' }}>{doc.status || 'unpaid'}</span>
            </div>
          </div>
          <div className="info-box">
            <div className="info-box-title">{L('Supplier Information', 'ข้อมูลผู้ขาย')}</div>
            <div className="info-row">
        ...
        <div className="signature-section">
          ...
          <div className="signature-box">
            ...
            <div className="signature-sublabel">{L('Finance', 'ฝ่ายการเงิน')}</div>
          </div>
          <div className="signature-box">
            <div className="signature-line"></div>
