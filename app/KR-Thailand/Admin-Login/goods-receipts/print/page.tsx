...
function GoodsReceiptPrintContent() {
  ...
  useEffect(() => {
    const idOrNo = grID || grNo
    if (!idOrNo) return
    ;(async () => {
      try {
        let res = await fetch(`/api/goods-receipts?grID=${encodeURIComponent(grID)}`)
        let j = await res.json().catch(() => null)
        if (!(res.ok && j && j.success && j.goodsReceipt)) {
          res = await fetch(`/api/goods-receipts?grNo=${encodeURIComponent(grNo || grID)}`)
          j = await res.json().catch(() => null)
        }
        if (j && j.success) {
          const q = j.goodsReceipt || j.goodsReceipts?.[0] || null
        ...
        console.error('Failed to load goods receipt for print', err)
      }
    })()
  }, [grID, grNo])
  ...
  useEffect(() => {
    ...
    const key = `print_count:goods-receipt:${grID || grNo || 'unknown'}`
  ...
  if (!grID && !grNo) return <div style={{ padding: 20 }}>Missing grID or grNo</div>
      ...
      <div className="a4-page">
        <div className="header-row">
          ...
          <div className="doc-title">
            <h1>{L('GOODS RECEIPT', 'ใบรับสินค้า')}</h1>
            <h2>{L('Receiving Document', 'เอกสารรับสินค้า')}</h2>
          </div>
        </div>

        <div className="info-section">
          <div className="info-box">
            <div className="info-box-title">{L('Goods Receipt Information', 'ข้อมูลใบรับสินค้า')}</div>
            <div className="info-row">
              <span className="info-label">{L('GR No:', 'เลขที่:')}</span>
              <span className="info-value" style={{ fontWeight: 700 }}>{doc.grNo || doc.gr_no || '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Date:', 'วันที่:')}</span>
              <span className="info-value">{doc.grDate || doc.date ? new Date(doc.grDate || doc.date).toLocaleDateString(selectedLang === 'th' ? 'th-TH' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('PO No:', 'ใบสั่งซื้อ:')}</span>
              <span className="info-value">{doc.po_no || doc.reference || '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Status:', 'สถานะ:')}</span>
              <span className="info-value" style={{ color: doc.status === 'received' ? '#16a34a' : '#666' }}>{doc.status || 'pending'}</span>
            </div>
          </div>
          <div className="info-box">
