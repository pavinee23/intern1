...
function StockAdjustmentPrintContent() {
  ...
  useEffect(() => {
    const idOrNo = saID || saNo
    if (!idOrNo) return
    ;(async () => {
      try {
        let res = await fetch(`/api/stock-adjustments?saID=${encodeURIComponent(saID)}`)
        let j = await res.json().catch(() => null)
        if (!(res.ok && j && j.success && j.stockAdjustment)) {
          res = await fetch(`/api/stock-adjustments?saNo=${encodeURIComponent(saNo || saID)}`)
          j = await res.json().catch(() => null)
        }
        if (j && j.success) {
          const q = j.stockAdjustment || j.stockAdjustments?.[0] || null
        ...
        console.error('Failed to load stock adjustment for print', err)
      }
    })()
  }, [saID, saNo])
  ...
  useEffect(() => {
    ...
    const key = `print_count:stock-adjustment:${saID || saNo || 'unknown'}`
  ...
  if (!saID && !saNo) return <div style={{ padding: 20 }}>Missing saID or saNo</div>
  ...
  const adjustmentType = doc.adjustment_type || doc.type || '-'
  const reason = doc.reason || '-'
  const location = doc.location || doc.warehouse || '-'
  const adjustedBy = doc.adjusted_by || doc.adjustedBy || '-'
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .doc-title h1 { font-size: 22pt; font-weight: 700; color: #d97706; margin: 0 0 4px 0; }
        ...
        .items-table th { background: #d97706; color: white; padding: 8px 10px; text-align: left; font-weight: 600; }
        ...
        .summary-row.total { font-weight: 700; font-size: 12pt; color: #d97706; border-top: 2px solid #d97706; border-bottom: none; padding-top: 10px; margin-top: 4px; }
      ...
      <div className="a4-page">
        <div className="header-row">
          ...
          <div className="doc-title">
            <h1>{L('STOCK ADJUSTMENT', 'ใบปรับสต๊อค')}</h1>
            <h2>{L('Adjustment Document', 'เอกสารปรับสต๊อก')}</h2>
          </div>
        </div>

        <div className="info-section">
          <div className="info-box">
            <div className="info-box-title">{L('Adjustment Information', 'ข้อมูลการปรับปรุง')}</div>
            <div className="info-row">
              <span className="info-label">{L('Adjust No:', 'เลขที่:')}</span>
              <span className="info-value" style={{ fontWeight: 700 }}>{doc.saNo || doc.sa_no || '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Date:', 'วันที่:')}</span>
              <span className="info-value">{doc.saDate || doc.date ? new Date(doc.saDate || doc.date).toLocaleDateString(selectedLang === 'th' ? 'th-TH' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Type:', 'ประเภท:')}</span>
              <span className="info-value">{adjustmentType}</span>
            </div>
            <div className="info-row">
          ...
          <div className="info-box">
            <div className="info-box-title">{L('Details', 'รายละเอียด')}</div>
            <div className="info-row">
              <span className="info-label">{L('Location:', 'สถานที่:')}</span>
              <span className="info-value" style={{ fontWeight: 600 }}>{location}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Adjusted By:', 'ผู้ปรับปรุง:')}</span>
              <span className="info-value">{adjustedBy}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Reason:', 'เหตุผล:')}</span>
        ...
        <div className="signature-section">
          ...
          <div className="signature-box">
            <div className="signature-line"></div>
            <div className="signature-label">{L('Checked By', 'ผู้ตรวจสอบ')}</div>
            <div className="signature-sublabel">{L('Inventory', 'ตรวจสต๊อก')}</div>
          </div>
          <div className="signature-box">
            <div className="signature-line"></div>
