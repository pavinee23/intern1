...
function PaymentVoucherPrintContent() {
  ...
  useEffect(() => {
    const idOrNo = pvID || pvNo
    if (!idOrNo) return
    ;(async () => {
      try {
        let res = await fetch(`/api/payment-vouchers?pvID=${encodeURIComponent(pvID)}`)
        let j = await res.json().catch(() => null)
        if (!(res.ok && j && j.success && j.paymentVoucher)) {
          res = await fetch(`/api/payment-vouchers?pvNo=${encodeURIComponent(pvNo || pvID)}`)
          j = await res.json().catch(() => null)
        }
        if (j && j.success) {
          const q = j.paymentVoucher || j.paymentVouchers?.[0] || null
        ...
        console.error('Failed to load payment voucher for print', err)
      }
    })()
  }, [pvID, pvNo])
  ...
  useEffect(() => {
    ...
    const key = `print_count:payment-voucher:${pvID || pvNo || 'unknown'}`
  ...
  if (!pvID && !pvNo) return <div style={{ padding: 20 }}>Missing pvID or pvNo</div>
  ...
  const payeeName = doc.payee_name || doc.supplier_name || doc.payee || '-'
  const payeeAddress = doc.payee_address || doc.address || '-'
  const payeePhone = doc.payee_phone || doc.phone || '-'
  const payeeEmail = doc.payee_email || doc.email || '-'
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .doc-title h1 { font-size: 22pt; font-weight: 700; color: #ca8a04; margin: 0 0 4px 0; }
        ...
        .items-table th { background: #ca8a04; color: white; padding: 8px 10px; text-align: left; font-weight: 600; }
        ...
        .summary-row.total { font-weight: 700; font-size: 12pt; color: #ca8a04; border-top: 2px solid #ca8a04; border-bottom: none; padding-top: 10px; margin-top: 4px; }
      ...
      <div className="a4-page">
        <div className="header-row">
          ...
          <div className="doc-title">
            <h1>{L('PAYMENT VOUCHER', 'ใบชำระเงิน')}</h1>
            <h2>{L('Payment Document', 'เอกสารชำระเงิน')}</h2>
          </div>
        </div>

        <div className="info-section">
          <div className="info-box">
            <div className="info-box-title">{L('Payment Voucher Information', 'ข้อมูลใบชำระเงิน')}</div>
            <div className="info-row">
              <span className="info-label">{L('PV No:', 'เลขที่:')}</span>
              <span className="info-value" style={{ fontWeight: 700 }}>{doc.pvNo || doc.pv_no || '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Date:', 'วันที่:')}</span>
              <span className="info-value">{doc.pvDate || doc.date ? new Date(doc.pvDate || doc.date).toLocaleDateString(selectedLang === 'th' ? 'th-TH' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Payment Method:', 'วิธีชำระ:')}</span>
              <span className="info-value">{doc.payment_method || doc.method || '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Status:', 'สถานะ:')}</span>
              <span className="info-value" style={{ color: doc.status === 'paid' ? '#16a34a' : '#666' }}>{doc.status || 'pending'}</span>
            </div>
          </div>
          <div className="info-box">
            <div className="info-box-title">{L('Payee Information', 'ข้อมูลผู้รับเงิน')}</div>
            <div className="info-row">
              <span className="info-label">{L('Name:', 'ชื่อ:')}</span>
              <span className="info-value" style={{ fontWeight: 600 }}>{payeeName}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Address:', 'ที่อยู่:')}</span>
              <span className="info-value">{payeeAddress}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Phone:', 'โทรศัพท์:')}</span>
              <span className="info-value">{payeePhone}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Email:', 'อีเมล:')}</span>
              <span className="info-value">{payeeEmail}</span>
            </div>
          </div>
        </div>
        ...
        <div className="signature-section">
          <div className="signature-box">
            ...
            <div className="signature-sublabel">{L('Accounting', 'ฝ่ายบัญชี')}</div>
          </div>
          <div className="signature-box">
            <div className="signature-line"></div>
          ...
          <div className="signature-box">
            ...
            <div className="signature-sublabel">{L('Payee', 'ผู้รับ')}</div>
          </div>
        </div>
