...
export default function PayOtherPage(){
  const{L}=useLang()
  ...
  const load=async()=>{const r=await fetch('/api/accounting/payment-vouchers'+"?voucher_type=pay&party_type=other"+(search?'&q='+encodeURIComponent(search):''));const d=await r.json();if(d.ok)setData(d.data||[])}
  useEffect(()=>{load()},[])
  ...
  return(<AccWindow title={L('Other Payments','จ่ายเงินอื่นๆ')}><div style={{padding:12}}>
