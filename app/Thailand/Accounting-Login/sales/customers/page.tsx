...
export default function CustomersPage() {
  ...
  const load = async () => {
    const res = await fetch('/api/accounting/customers' + (search ? '?q=' + encodeURIComponent(search) : ''))
      ...
      <div style={{ padding: 12 }}>
        ...
        {showForm && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: '#fff', border: '2px solid', border: '1px solid #d1d5db', borderRadius: 8, minWidth: 460, maxWidth: 560, width: '95%', boxShadow: '4px 4px 12px rgba(0,0,0,0.4)' }}>
              ...
              <div style={{ padding: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 12px' }}>
                {[
                  ['ชื่อ (ภาษาไทย) *', 'name_th', 'text', true],
                  ['ชื่อ (ภาษาอังกฤษ)', 'name_en', 'text', true],
                  ['เลขที่ผู้เสียภาษี', 'tax_id', 'text', false],
                  ['ชื่อผู้ติดต่อ', 'contact_name', 'text', false],
                  ['โทรศัพท์', 'phone', 'text', false],
                  ['อีเมล', 'email', 'email', false],
                  ['เครดิต (วัน)', 'credit_days', 'number', false],
                  ['วงเงินเครดิต (บาท)', 'credit_limit', 'number', false],
                ].map(([label, key, type, span]) => (
                  <div key={key as string} style={span ? { gridColumn: 'span 2' } : {}}>
