...
export default function SalesDepositPage(){
  const{L}=useLang()
  const[data,setData]=useState<any[]>([]);const[form,setForm]=useState<any>({doc_no:'',doc_date:new Date().toISOString().slice(0,10),customer_name:'',total:0,status:''});const[showForm,setShowForm]=useState(false)
  const[loading,setLoading]=useState(false);const[search,setSearch]=useState('');const[msg,setMsg]=useState('')
  const load=async()=>{const r=await fetch('/api/accounting/sales-invoices'+"?doc_type=deposit"+(search?'&q='+encodeURIComponent(search):''));const d=await r.json();if(d.ok)setData(d.data||[])}
  useEffect(()=>{load()},[])
  const save=async()=>{setLoading(true);setMsg('');const m=form.id?'PUT':'POST';const r=await fetch('/api/accounting/sales-invoices',{method:m,headers:{'Content-Type':'application/json'},body:JSON.stringify(form)});const d=await r.json();if(d.ok){setShowForm(false);setForm({doc_no:'',doc_date:new Date().toISOString().slice(0,10),customer_name:'',total:0,status:''});load();setMsg(L('Saved','บันทึกสำเร็จ'))}else setMsg('Error: '+d.error);setLoading(false)}
  ...
  return(<AccWindow title={L('Receive Deposit','รับเงินมัดจำ')}><div style={{padding:12}}>
    <div style={{display:'flex',gap:6,marginBottom:8,alignItems:'center',flexWrap:'wrap'}}>
      <button style={btn('#f3f4f6')} onClick={()=>{setForm({doc_no:'',doc_date:new Date().toISOString().slice(0,10),customer_name:'',total:0,status:''});setShowForm(true)}}>+ {L('New','เพิ่มใหม่')}</button>
    ...
    <div style={{overflowX:'auto',border:'1px solid #d1d5db',borderRadius:12,overflow:'hidden',boxShadow:'0 2px 6px rgba(0,0,0,0.08)'}}><table style={{width:'100%',borderCollapse:'collapse'}}>
      <thead><tr>{[L('Doc No','เลขที่'),L('Date','วันที่'),L('Customer','ลูกค้า'),L('Amount','จำนวนเงิน'),L('Status','สถานะ'),''].map((h,i)=><th key={i} style={th}>{h}</th>)}</tr></thead>
      <tbody>{data.length===0&&<tr><td colSpan={6} style={{...td,textAlign:'center',color:'#888',padding:20}}>{L('No data','ไม่มีข้อมูล')}</td></tr>}
        {data.map((r:any,i)=>(<tr key={r.id} style={{background:i%2?'#f5f5f5':'#fff'}}>
          <td style={td}>{r.doc_no}</td><td style={td}>{r.doc_date?.slice(0,10)}</td><td style={td}>{r.customer_name}</td><td style={{...td,textAlign:'right'}}>{fmt(r.total||0)}</td><td style={{...td,textAlign:'center'}}>{r.status}</td>
      ...
      <div style={{background:'#fff',border:'1px solid #d1d5db',borderRadius:12,boxShadow:'0 20px 60px rgba(0,0,0,0.3)',minWidth:420,maxWidth:520,width:'95%'}}>
        ...
        <div style={{padding:16,display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px 12px'}}>
          <div><div style={{fontSize:12,marginBottom:2}}>{L('Doc No','เลขที่')}</div><input style={inp} value={form.doc_no||''} onChange={e=>setForm(f2=>({...f2,doc_no:e.target.value}))} /></div>
              ...
              <div><div style={{fontSize:12,marginBottom:2}}>{L('Amount','จำนวนเงิน')}</div><input style={inp} type="number" value={form.total||''} onChange={e=>setForm(f2=>({...f2,total:+e.target.value||0}))} /></div>
