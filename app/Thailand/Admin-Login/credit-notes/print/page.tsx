...
function CreditNotePrintContent() {
  ...
  useEffect(() => {
    const idOrNo = cnID || cnNo
    if (!idOrNo) return
    ;(async () => {
      try {
        let res = await fetch(`/api/credit-notes?cnID=${encodeURIComponent(cnID)}`)
        let j = await res.json().catch(() => null)
        if (!(res.ok && j && j.success && j.creditNote)) {
          res = await fetch(`/api/credit-notes?cnNo=${encodeURIComponent(cnNo || cnID)}`)
          j = await res.json().catch(() => null)
        }
        if (j && j.success) {
          const q = j.creditNote || j.creditNotes?.[0] || null
        ...
        console.error('Failed to load credit note for print', err)
      }
    })()
  }, [cnID, cnNo])
  ...
  useEffect(() => {
    ...
    const key = `print_count:credit-note:${cnID || cnNo || 'unknown'}`
  ...
  if (!cnID && !cnNo) return <div style={{ padding: 20 }}>Missing cnID or cnNo</div>
  ...
  const customerAddress = doc.customer_address || doc.address || '-'
  const customerPhone = doc.customer_phone || doc.phone || '-'
  const customerEmail = doc.customer_email || doc.email || '-'
      ...
      <div className="a4-page">
        <div className="header-row">
          ...
          <div className="doc-title">
            <h1>{L('CREDIT NOTE', 'ใบลดหนี้')}</h1>
            <h2>{L('Credit Memo', 'เอกสารลดหนี้')}</h2>
          </div>
        </div>

        <div className="info-section">
          <div className="info-box">
            <div className="info-box-title">{L('Credit Note Information', 'ข้อมูลใบลดหนี้')}</div>
            <div className="info-row">
              <span className="info-label">{L('CN No:', 'เลขที่:')}</span>
              <span className="info-value" style={{ fontWeight: 700 }}>{doc.cnNo || doc.cn_no || '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Date:', 'วันที่:')}</span>
              <span className="info-value">{doc.cnDate || doc.date ? new Date(doc.cnDate || doc.date).toLocaleDateString(selectedLang === 'th' ? 'th-TH' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : '-'}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{L('Reference:', 'อ้างอิง:')}</span>
              <span className="info-value">{doc.reference || doc.invoice_no || '-'}</span>
            </div>
            <div className="info-row">
