...
export default function ExportPage() {
  ...
  const download = async (format: 'csv' | 'json') => {
    ...
    try {
      ...
      const res = await fetch('/research-development/export/route', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen bg-sky-50 p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow p-6">
        <h2 className="text-2xl font-bold mb-2">{locale === 'ko' ? '데이터 내보내기' : 'Export Data'}</h2>
        <p className="text-sm text-gray-600 mb-4">{locale === 'ko' ? '데이터를 CSV 또는 JSON 형식으로 내보낼 수 있습니다. AI 학습/전처리에 사용하세요.' : 'Export tables as CSV or JSON for AI training and data preparation.'}</p>
