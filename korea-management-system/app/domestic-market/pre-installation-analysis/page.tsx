...
const generateDocumentNumber = (): string => {
  ...
  return `DPIA-${year}-${month}${day}-${String(random).padStart(3, '0')}`;
};
...
const sampleAnalysis: AnalysisData = {
  id: 'DPIA-2026-0216-001', // Static initial value to prevent hydration error
  branch: 'Seoul/Gyeonggi',
  location: '서울 강남구 역삼동 본사 빌딩 - 메인 판넬',
  equipment: 'Fluke 1760 전력 품질 분석기',
  ...
  balance: 'Good',
  result: 'Recommended',
  recommendation: '전류 균형 상태 우수, KSAVE 즉시 설치 가능.',
  notes: '본사 전력 시스템 최적화 상태, 고효율 기대'
};
...
const analysisTranslations = {
  ...
  en: {
    ...
    sendToFlow: 'Connect to Flow System',
...
export default function PreInstallationAnalysisPage(): JSX.Element {
  ...
  const [selectedBranch, setSelectedBranch] = useState(locale === 'ko' ? '서울/경기' : 'Seoul/Gyeonggi');
  const [selectedCustomer, setSelectedCustomer] = useState(locale === 'ko' ? '서울 강남구 역삼동 본사 빌딩 - 메인 판넬' : 'Seoul Gangnam-gu Yeoksam-dong HQ Building - Main Panel');
  const [selectedEquipment, setSelectedEquipment] = useState(locale === 'ko' ? 'Fluke 1760 전력 품질 분석기' : 'Fluke 1760 Power Quality Analyzer');
  ...
  const [currentValues, setCurrentValues] = useState({
    L1: 192.3, // Phase A from 7-day measurement
    L2: 198.7, // Phase B from 7-day measurement  
    L3: 185.9, // Phase C from 7-day measurement
    N: 11.4
  });
  const [selectedProduct, setSelectedProduct] = useState('KSAVER 150KVA');
  const [selectedEngineer, setSelectedEngineer] = useState(locale === 'ko' ? '김종수' : 'Kim Jong-soo');
  ...
  const [printLanguage, setPrintLanguage] = useState<'ko' | 'en'>(locale === 'ko' ? 'ko' : 'en');
...
<body>
  ...
  <div class="report-header">
    ...
    <div class="report-subtitle" style="text-align: center;">${new Date().toLocaleDateString(printLanguage === 'ko' ? 'ko-KR' : 'en-US')}</div>
  </div>
  ...
  <div style="background: #f0f9ff; border: 2px solid #0ea5e9; border-radius: 8px; padding: 15px; margin: 20px 0; text-align: center; page-break-inside: avoid;">
    <div style="display: flex; align-items: center; justify-content: center; gap: 10px; margin-bottom: 10px;">
      ...
      <span style="color: #0ea5e9; font-weight: bold; font-size: 14px;">
        ${printLanguage === 'ko' ? '인쇄 미리보기' : 'Print Preview'}
      </span>
    </div>
    <button onclick="window.print()" style="background: #0ea5e9; color: white; border: none; padding: 10px 20px; border-radius: 6px; font-weight: bold; cursor: pointer; font-size: 12px; transition: all 0.2s;">
      🖨️ ${printLanguage === 'ko' ? '지금 인쇄하기' : 'Print Now'}
    </button>
    <div style="margin-top: 8px; font-size: 10px; color: #64748b;">
      ${printLanguage === 'ko' ? '이 미리보기를 확인한 후 인쇄 버튼을 클릭하세요' : 'Review this preview and click the print button'}
    </div>
  </div>
  ...
  <h2>${printLanguage === 'ko' ? '추가 테스트 필요' : 'Additional Tests Required'}</h2>
  ...
  <div style="background: #eff6ff; border: 2px solid #3b82f6; border-radius: 6px; padding: 12px; margin: 15px 0; font-size: 9px;">
    <h3 style="margin: 0 0 8px 0; color: #1e40af; font-size: 11px; font-weight: bold;">📅 ${printLanguage === 'ko' ? '예정된 후속 조치' : 'Scheduled Follow-up'}</h3>
    <div style="color: #1e3a8a; line-height: 1.5;">
      <p style="margin: 2px 0;"><strong>${printLanguage === 'ko' ? '날짜' : 'Date'}:</strong> ${analysis.scheduledFollowUp.date}</p>
      <p style="margin: 2px 0;"><strong>${printLanguage === 'ko' ? '기술자' : 'Technician'}:</strong> ${analysis.scheduledFollowUp.technician}</p>
      <p style="margin: 2px 0;"><strong>${printLanguage === 'ko' ? '우선순위' : 'Priority'}:</strong> ${analysis.scheduledFollowUp.priority}</p>
      <p style="margin: 2px 0;"><strong>${printLanguage === 'ko' ? '예상 시간' : 'Expected Duration'}:</strong> ${analysis.scheduledFollowUp.expectedDuration}</p>
    </div>
  </div>
  ` : ''}
  ` : ''}
  ...
  <div class="report-footer">
    <p style="color: #6b7280; font-size: 9px; margin-bottom: 5px;">
      ${printLanguage === 'ko' ? 'K 에너지 세이브 주식회사 (Zera 그룹)' : 'K Energy Save Co., Ltd. (Group of Zera)'}
    </p>
    <p style="color: #9ca3af; font-size: 9px;">
      ${printLanguage === 'ko' ? '생성일' : 'Generated on'}: ${new Date().toLocaleString(printLanguage === 'ko' ? 'ko-KR' : 'en-US')}
    </p>
...
Flow System: ${flowUrl}
  ...
  const branches = [
    { id: 'seoul', name: locale === 'ko' ? '서울/경기' : 'Seoul/Gyeonggi', code: 'SEL' },
    { id: 'busan', name: locale === 'ko' ? '부산/경남' : 'Busan/Gyeongnam', code: 'BSN' },
    { id: 'daegu', name: locale === 'ko' ? '대구/경북' : 'Daegu/Gyeongbuk', code: 'DGU' },
    { id: 'daejeon', name: locale === 'ko' ? '대전/충청' : 'Daejeon/Chungcheong', code: 'DJN' },
    { id: 'gwangju', name: locale === 'ko' ? '광주/전라' : 'Gwangju/Jeolla', code: 'GWJ' },
    { id: 'incheon', name: locale === 'ko' ? '인천/강원' : 'Incheon/Gangwon', code: 'ICN' },
    { id: 'jeju', name: locale === 'ko' ? '제주' : 'Jeju', code: 'JJU' }
  ];

  const customersByBranch = {
    'Seoul/Gyeonggi': [
      locale === 'ko' ? '서울 강남구 역삼동 본사 빌딩 - 메인 판넬' : 'Seoul Gangnam-gu Yeoksam-dong HQ Building - Main Panel',
      locale === 'ko' ? '경기 수원시 삼성전자 인근 사업소 - UPS실' : 'Gyeonggi Suwon Samsung Electronics Nearby Office - UPS Room',
      locale === 'ko' ? '서울 구로구 가산디지털단지 - 데이터센터' : 'Seoul Guro-gu Gasan Digital Complex - Data Center',
      locale === 'ko' ? '경기 판교시 LCD 공장 - 생산라인' : 'Gyeonggi Paju LCD Factory - Production Line'
    ],
    'Busan/Gyeongnam': [
      locale === 'ko' ? '부산 기장군 정관공장 - 생산라인 A' : 'Busan Gijang-gun Jeonggwan Factory - Production Line A',
      locale === 'ko' ? '경남 창원시 기계 공장 - 메인 배전반' : 'Gyeongnam Changwon Machinery Factory - Main Panel',
      locale === 'ko' ? '부산 해운대구 항만 물류센터 - 냉동창고' : 'Busan Haeundae-gu Port Logistics Center - Cold Storage'
    ],
    'Daegu/Gyeongbuk': [
      locale === 'ko' ? '대구 달성군 산업단지 지사 - 사무동' : 'Daegu Dalseong-gun Industrial Complex Branch - Office Building',
      locale === 'ko' ? '경북 구미시 전자부품 공장 - 조립라인' : 'Gyeongbuk Gumi Electronic Parts Factory - Assembly Line',
      locale === 'ko' ? '대구 북구 섬유 공장 - 동력실' : 'Daegu Buk-gu Textile Factory - Power Room'
    ],
    'Daejeon/Chungcheong': [
      locale === 'ko' ? '대전 유성구 대덕연구단지 - R&D 센터' : 'Daejeon Yuseong-gu Daedeok Research Complex - R&D Center',
      locale === 'ko' ? '충남 아산시 반도체 공장 - 클린룸' : 'Chungnam Asan Semiconductor Factory - Cleanroom',
      locale === 'ko' ? '대전 서구 청사동 본사 - 메인 배전반' : 'Daejeon Seo-gu Cheongsa-dong HQ - Main Distribution'
    ],
    'Gwangju/Jeolla': [
      locale === 'ko' ? '광주 광산구 물류센터 - 냉동창고' : 'Gwangju Gwangsan-gu Logistics Center - Cold Storage',
      locale === 'ko' ? '전남 여수시 화학공장 - 생산라인' : 'Jeonnam Yeosu Chemical Factory - Production Line',
      locale === 'ko' ? '광주 북구 첨단산업단지 - 전력실' : 'Gwangju Buk-gu High-tech Industrial Complex - Power Room'
    ],
    'Incheon/Gangwon': [
      locale === 'ko' ? '인천 서구 검단 신규 설치현장 - 배전반' : 'Incheon Seo-gu Geomdan New Installation Site - Distribution Panel',
      locale === 'ko' ? '강원 원주시 기업도시 - 사무동' : 'Gangwon Wonju Enterprise City - Office Building',
      locale === 'ko' ? '인천 남동구 공항 물류터미널 - 화물센터' : 'Incheon Namdong-gu Airport Logistics Terminal - Cargo Center'
    ],
    'Jeju': [
      locale === 'ko' ? '제주 서귀포시 태양광 발전소 - 관리동' : 'Jeju Seogwipo Solar Power Plant - Management Building',
      locale === 'ko' ? '제주시 청정바람 발전단지 - 변전소' : 'Jeju City Clean Wind Power Complex - Substation',
      locale === 'ko' ? '제주시 청정과학센터 - 연구동' : 'Jeju City Clean Science Center - Research Building'
    ]
  };

  const equipmentByBranch = {
    'Seoul/Gyeonggi': [
      locale === 'ko' ? 'Fluke 1760 전력 품질 분석기' : 'Fluke 1760 Power Quality Analyzer',
      locale === 'ko' ? 'Hioki PW3198 Power Analyzer' : 'Hioki PW3198 Power Analyzer',
      locale === 'ko' ? 'Chauvin Arnoux CA 8335' : 'Chauvin Arnoux CA 8335'
    ],
    'Busan/Gyeongnam': [
      locale === 'ko' ? 'Fluke 438-II Motor Analyzer' : 'Fluke 438-II Motor Analyzer',
      locale === 'ko' ? 'Hioki PW3198 Power Analyzer' : 'Hioki PW3198 Power Analyzer',
      locale === 'ko' ? 'Yokogawa CW240 Clamp Power Meter' : 'Yokogawa CW240 Clamp Power Meter'
    ],
    'Daegu/Gyeongbuk': [
      locale === 'ko' ? 'Megger MIT1525 Insulation Tester' : 'Megger MIT1525 Insulation Tester',
      locale === 'ko' ? 'Fluke 1760 전력 품질 분석기' : 'Fluke 1760 Power Quality Analyzer',
      locale === 'ko' ? 'Chauvin Arnoux CA 8335' : 'Chauvin Arnoux CA 8335'
    ],
    'Daejeon/Chungcheong': [
    ...
    'Gwangju/Jeolla': [
      locale === 'ko' ? 'Yokogawa CW240 Clamp Power Meter' : 'Yokogawa CW240 Clamp Power Meter',
      locale === 'ko' ? 'Kikusui KHA1000 Harmonic Analyzer' : 'Kikusui KHA1000 Harmonic Analyzer',
      locale === 'ko' ? 'Hioki PW3198 Power Analyzer' : 'Hioki PW3198 Power Analyzer'
    ],
    'Incheon/Gangwon': [
      locale === 'ko' ? 'AEMC 2135.36 Clamp-on Ground Tester' : 'AEMC 2135.36 Clamp-on Ground Tester',
      locale === 'ko' ? 'Fluke 1760 전력 품질 분석기' : 'Fluke 1760 Power Quality Analyzer',
      locale === 'ko' ? 'Chauvin Arnoux CA 8335' : 'Chauvin Arnoux CA 8335'
    ],
    'Jeju': [
  ...
  const availableEngineers = [
    locale === 'ko' ? '김종수' : 'Kim Jong-soo',
    locale === 'ko' ? '박민호' : 'Park Min-ho',
    locale === 'ko' ? '이상철' : 'Lee Sang-cheol',
    locale === 'ko' ? '최영수' : 'Choi Young-soo',
    locale === 'ko' ? '정현우' : 'Jeong Hyun-woo',
    locale === 'ko' ? '김태홍' : 'Kim Tae-hong',
    locale === 'ko' ? '서영민' : 'Seo Young-min',
    locale === 'ko' ? '강도현' : 'Kang Do-hyeon'
  ];
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    ...
                    <tr className="hover:bg-gray-50 bg-blue-50">
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex flex-col">
                          <span className="text-xs font-bold text-blue-900">{analysis.id}</span>
                          <span className="mt-1 px-2 py-0.5 text-xs bg-blue-600 text-white rounded-full inline-block w-fit">
                            {locale === 'ko' ? '현재' : 'Current'}
                          </span>
                        </div>
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap">
                        <div className="text-xs font-semibold text-gray-900">{selectedBranch}</div>
                      </td>
                      <td className="px-3 py-3">
                        <div className="text-xs text-gray-700 max-w-xs truncate" title={selectedCustomer}>
                          {selectedCustomer}
                        </div>
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap">
                        <span className="text-xs text-gray-700">{analysis.datetime}</span>
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap">
                        <div className="flex items-center space-x-1">
                          <CheckCircle className="w-4 h-4 text-green-600 flex-shrink-0" />
                          <span className="text-xs font-medium text-green-700">
                            {locale === 'ko' ? '설치 권장' : 'Recommended'}
                          </span>
                        </div>
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap text-center">
                        <div className="flex items-center justify-center space-x-2">
                          ...
                          <button 
                            ...
                            onClick={handleSendToFlowSystem}
                            title={locale === 'ko' ? 'Flow 연결' : 'Connect to Flow'}
                          >
                            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                              <path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z" />
                              <path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z" />
                    ...
                    <tr className="hover:bg-gray-50">
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className="text-xs font-semibold text-gray-900">DPIA-2026-0215-089</span>
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap">
                        <div className="text-xs font-semibold text-gray-900">{locale === 'ko' ? '부산/경남' : 'Busan/Gyeongnam'}</div>
                      </td>
                      <td className="px-3 py-3">
                        <div className="text-xs text-gray-700 max-w-xs truncate" title={locale === 'ko' ? '부산 기장군 정관공장 - 생산라인 A' : 'Busan Gijang-gun Jeonggwan Factory - Production Line A'}>
                          {locale === 'ko' ? '부산 기장군 정관공장 - 생산라인 A' : 'Busan Gijang-gun Jeonggwan Factory - Production Line A'}
                        </div>
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap">
                        <span className="text-xs text-gray-700">2026-02-15 10:20</span>
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap">
                        <div className="flex items-center space-x-1">
                          <CheckCircle className="w-4 h-4 text-green-600 flex-shrink-0" />
                    ...
                    <tr className="hover:bg-gray-50">
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className="text-xs font-semibold text-gray-900">DPIA-2026-0214-067</span>
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap">
                        <div className="text-xs font-semibold text-gray-900">{locale === 'ko' ? '대전/충청' : 'Daejeon/Chungcheong'}</div>
                      </td>
                      <td className="px-3 py-3">
                        <div className="text-xs text-gray-700 max-w-xs truncate" title={locale === 'ko' ? '대전 유성구 대덕연구단지 - R&D 센터' : 'Daejeon Yuseong-gu Daedeok Research Complex - R&D Center'}>
                          {locale === 'ko' ? '대전 유성구 대덕연구단지 - R&D 센터' : 'Daejeon Yuseong-gu Daedeok Research Complex - R&D Center'}
                        </div>
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap">
                        <span className="text-xs text-gray-700">2026-02-14 14:45</span>
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap">
                        <div className="flex items-center space-x-1">
                          <AlertTriangle className="w-4 h-4 text-yellow-600 flex-shrink-0" />
                          <span className="text-xs font-medium text-yellow-700">
                            {locale === 'ko' ? '추가 분석' : 'Further Analysis'}
                          </span>
                        </div>
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap text-center">
